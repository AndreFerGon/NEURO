...
class ColorSwitchingSquare:
    ...
    def update(self):
        if self.timer_active:
            current_time = pygame.time.get_ticks()
            if current_time - self.timer_start_time >= 1500:  # 1000 milliseconds = 1 second
...
def main():
    ...
    server_socket.bind(('localhost', 12345))  # Set your desired host and port
    ...
    # Define positions and intervals for each checkerboard square
    checkerboard_info = [
    ...
    while running:
        ...
        try:
            ...
            # Find the corresponding color-switching square based on the received message
            for square_data in color_squares:
                if message == square_data["id"]:
                    square_data["square"].set_permanent_green()  # Set the square to green permanently
        except socket.error:
            pass
