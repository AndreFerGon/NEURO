...
class CheckerboardSquare:
    def __init__(self, rect, num_rows, num_cols, toggle_interval):
        ...
        self.showing = True  # Start by showing the squares
    ...
    def update(self):
        ...
        if current_time - self.last_toggle_time >= self.toggle_interval:
            ...
            # Toggle the colors of the squares
            for row in range(self.num_rows):
                for col in range(self.num_cols):
                    if self.colors[row][col] == WHITE:
                        self.colors[row][col] = BLACK
                    elif self.colors[row][col] == BLACK:
...
def main():
    ...
    while running:
        ...
        if delay_complete:
            ...
            if elapsed_time <= 6000:  # Adjust this duration as needed
            ...
            # Display stopwatch during delay
